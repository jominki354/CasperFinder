...
## [2026-02-11] v0.0.4: 뱃지 정확도 개선 및 UX 보안
- **뱃지 카운트 수정**: 현재 탭 무관하게 항상 카운트 증가하도록 변경.
- **작업표시줄 깜박임 수정**: `FlashWindow` → `FlashWindowEx` + `GetParent()` HWND로 정확한 깜박임 구현.
- **더미 데이터 이스터에그**: `from 사슴` 라벨 7회 클릭 시 더미 데이터 생성 버튼 토글 (일반 사용자에게 숨김).
- **업데이트 확인 UI 복구**: 설정 페이지 내 `업데이트 확인` 버튼 및 `다운로드 및 설치` 인라인 UI 복원.
