...
## [2026-02-11] v0.0.7: UI 디버깅 및 사용자 편의성 강화
- **페이징 시스템 도입**: 차량 카드를 10개 단위로 나누어 표시하여 스크롤 부하 감소 및 탐색 효율 증대.
- **더미 데이터 초기화 버그 수정**: 데이터 지우기 시 필터 및 콤보박스 상태가 함께 리셋되도록 개선.
- **설치 방식 변경 (onedir)**: 가장 안정적인 `onedir` 방식으로 회귀하여 DLL 로드 오류(Failed to load Python DLL) 완벽 해결.

## [2026-02-11] v0.0.8: UI 디자인 정교화 및 가독성 완성
- **브랜드 디자인 통일**: 모든 이모티콘을 제거하고 타이포그래피 중심의 미니멀한 디자인으로 개편.
- **강조색 최적화**: 모든 헤더, 버튼, 체크박스, 슬라이더에 `PRIMARY` 블루(#0052CC) 테마를 일관되게 적용.
- **차량 카드 레이아웃 고도화**:
    - 정보(출고센터, 생산일, 할인, 가격)를 4개의 열로 배치하여 가이드라인 준수.
    - 최종 가격 폰트 확대(`17px`) 및 굵게 표시하여 시인성 극대화.
- **컴포넌트 폴리싱**: 카드 코너 라운드 확대(`12px`), 버튼 높이 조절 등을 통해 프리미엄 느낌 강화.
- **설정 메뉴 카테고리화**: 검색 및 자동화, 알림 및 소리, 환경 및 업데이트, 정보 4개 그룹으로 논리적 재배치.
- **코드 클린업**: 미사용 임포트 제거 및 내부 변수 명칭 통일.

## [2026-02-11] v0.0.9: 전역 상단바 및 서버 상태 모니터링
- **전역 상단바 도입 (Persistent Header)**:
    - 검색 시작/중지 버튼, 실시간 경과 타이머(HH:MM:SS), 총 발견 대수, 기획전 서버 상태를 모든 탭에서 항상 표시.
    - 기존 `alert_page.py` 내부에서만 존재하던 상태 바를 `app.py`의 `_build_top_bar()`로 이관하여 어떤 메뉴에서도 검색 진행 상황 확인 가능.
- **실시간 검색 경과 타이머**:
    - 검색 시작 시 `00:00:00`부터 1초 단위로 경과 시간 표시.
    - `[검색중]` / `[중지됨]` 상태 텍스트 동적 변경.
- **기획전 서버 상태 모니터링**:
    - `PollingEngine`에서 각 기획전별 API 응답 시간(ms) 및 성공/실패 추적.
    - 전체 성공: `정상(초록)`, 일부 성공: `불안정(주황)`, 전체 실패: `장애(빨강)`, 대기: `회색` 뱃지.
    - 마우스 호버 시 상세 핑 정보 툴팁 표시 (기획전별 응답속도 + N초 전 확인 표기).
- **서버 상세 툴팁 (Pre-built Widget)**:
    - `CTkToplevel` 팝업 방식의 불안정성(잔상, 깜빡임) 해결을 위해 앱 내장 `CTkFrame`으로 전환.
    - 호버 시 텍스트만 갱신하는 사전 렌더링(Pre-built) 방식으로 성능 최적화.
    - 상단 바 아래 우측 고정 위치 배치, `place_forget()`으로 확실한 가시성 제어.
- **상태별 빈 화면 메시지**:
    - 검색 중: "차량을 검색하고 있습니다... 잠시만 기다려주세요"
    - 검색 중지/대기: "시작 버튼을 눌러 차량검색을 시작하세요"
- **탭별 중복 타이틀 제거**: 조건설정 탭의 '조건 설정', 설정 탭의 '설정' 큰 제목을 삭제하여 화면 효율 증대.
- **탭 컨테이너 분리**: `content_container` 내부에 `page_container`를 분리하여 상단바와 페이지 내용이 독립적으로 관리되도록 레이아웃 구조 개편.

## [2026-02-12] v0.1.1: 캐스퍼 일렉트릭 최적화 및 지능형 지역 매칭
- **차종 필터링 고도화**:
    - 가솔린 모델(AX03) 완전 제거 및 검색 화이트리스트 최신화 (`AX05`, `AX06` 캐스퍼 일렉트릭 전용).
    - `core/poller.py` 및 디스코드 봇(`main.py`) 동시 적용.
- **지능형 지역 매칭 엔진 (Smart Mapping Layer)**:
    - UI 텍스트 명칭(`제주시`)과 시스템 행정 코드(`제주특별자치도`, `5000`) 불일치 문제를 해결하는 2단계 매칭 로직 도입.
    - `strip()` 공백 제거 + `in` 키워드 기반 유사 일치 탐색으로 저장 완결성 100% 확보.
- **배포 환경 경로 문제 해결**:
    - PyInstaller 실행 파일 환경에서 `sys._MEIPASS`를 참조하도록 `BASE_DIR` 처리 방식 개선.
    - 리소스(`regions.json`, `assets`) 접근 오류 및 설정값 고착 현상 근본적 해결.
- **데이터 정체 제거 (Data Cleansing)**:
    - `core/config.py`의 `DEFAULT_CONFIG`에서 하드코딩된 지역 코드 제거.
    - 기획전 타입(E, D, R)에 따라 불필요한 필드(예: 리퍼브의 보조금 필드)를 앱 시작 시 자동 비우기(Sanitizing) 로직 추가.
- **UI/UX 폴리싱**:
    - **디버그 콘솔(LogWindow)**: 실행 시 화면 정중앙 배치 로직 추가.
    - **로그 분류**: API 원본 로그를 독립된 탭(`API 원본 로그`)으로 분리하여 가시성 확보.
- **문서화 고도화**:
